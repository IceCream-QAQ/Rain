...
import rain.di.ClassContext
import rain.di.Config
import rain.di.impl.BeanFactoryClassContext
import rain.di.impl.BindableClassContext.Companion.putBinds
import rain.di.impl.ContextImpl
import rain.di.impl.NoInstanceClassContext
import rain.function.findClassByPackage
import rain.function.isBean
import rain.function.mapMap
...
//        val beanFactoryList = ArrayList<Class<out BeanFactory<*>>>()
        ...
        beanFactoryMap.forEach { (type, factoryType) ->
            BeanFactoryClassContext(
                context,
                context.findContext(factoryType) as ClassContext<BeanFactory<Any>>,
                type as Class<Any>
            )
    ...
    open fun addLoadItem(
        ...
        loadItemsMap.getOrPut(
            Class.forName(
                loadBy.value.java.name,
                false,
                classLoader
            ) as Class<out Loader>
        ) { HashMap() }
